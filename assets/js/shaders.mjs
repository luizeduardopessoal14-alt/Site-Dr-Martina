/* esm.sh - @paper-design/shaders@0.0.81 */
...
}`;function Ee(o){return"paperShaderMount"in o}function Pe(){let o=navigator.userAgent.toLowerCase();return o.includes("safari")&&!o.includes("chrome")&&!o.includes("android")}function Te(o){let e=visualViewport?.scale??1,a=visualViewport?.width??window.innerWidth,r=window.innerWidth-o.documentElement.clientWidth,u=e*a+r,l=outerWidth/u,c=Math.round(100*l);return c%5===0?c/100:c===33?1/3:c===67?2/3:c===133?4/3:l}var Ge={fit:"contain",scale:1,rotation:0,offsetX:0,offsetY:0,originX:.5,originY:.5,worldWidth:0,worldHeight:0},Ne={fit:"none",scale:1,rotation:0,offsetX:0,offsetY:0,originX:.5,originY:.5,worldWidth:0,worldHeight:0},Qe={none:0,contain:1,cover:2};var d=`
...
`;var ge={maxColorCount:10},We=`#version 300 es
precision mediump float;
...
`;var le={maxColorCount:10,maxNoiseIterations:8},Ye=`#version 300 es
precision mediump float;
...
vec2 fbm(vec2 n0, vec2 n1) {
  vec2 total = vec2(0.0);
  float amplitude = .4;
  for (int i = 0; i < ${le.maxNoiseIterations}; i++) {
...
void main() {
  ...
  for (int i = ${le.maxColorCount} - 2; i >= 0; i--) {
...
`;var ve={maxColorCount:10},He=`#version 300 es
precision mediump float;
...
`,Xe={circle:0,diamond:1,square:2,triangle:3};var he={maxColorCount:10},Ke=`#version 300 es
precision mediump float;
...
`;var ne={maxColorCount:8,maxBallsCount:20},je=`#version 300 es
precision mediump float;
...
`;var xe={maxColorCount:5},Ze=`#version 300 es
precision mediump float;
...
`;var _e={maxColorCount:10},eo=`#version 300 es
precision mediump float;
...
`,oo={checks:0,stripes:1,edge:2};var ye={maxColorCount:5},to=`#version 300 es
precision mediump float;
...
`;var be={maxColorCount:10},io=`#version 300 es
precision mediump float;
...
`,so={simplex:1,warp:2,dots:3,wave:4,ripple:5,swirl:6,sphere:7},lo={random:1,"2x2":2,"4x4":3,"8x8":4};var Ae={maxColorCount:7},no=`#version 300 es
...
`,co={wave:1,dots:2,truchet:3,corners:4,ripple:5,blob:6,sphere:7};var ce={maxColorCount:5,maxSpots:4},uo=`#version 300 es
precision lowp float;
...
void main() {
  ...
  fragColor = vec4(color, opacity);
}`,fo={auto:0,square:1};var ue={maxColorCount:7},mo=`#version 300 es
precision lowp float;
...
`;var we={maxColorCount:10},go=`#version 300 es
precision mediump float;
...
`;var fe={maxCrumpleCount:15},vo=`#version 300 es
precision mediump float;
...
vec4 getCrumples(vec2 uv) {
  ...
  vec4 seeds[${fe.maxCrumpleCount}];
  for (int i = 0; i < ${fe.maxCrumpleCount}; i++) {
...
`,_o={lines:1,linesIrregular:2,wave:3,zigzag:4,pattern:5},yo={prism:1,lens:2,contour:3,cascade:4,flat:5};var bo=`#version 300 es
precision mediump float;
...
`;var Re={maxSamples:50},Ao=`#version 300 es
precision mediump float;
...
`;var Be={maxColorCount:10},Co=`#version 300 es
...
`;function wo(o){let e=document.createElement("canvas"),a=1e3;return new Promise((r,u)=>{let l=new Image;l.crossOrigin="anonymous",l.addEventListener("load",()=>{(typeof o=="string"?o.endsWith(".svg"):o.type==="image/svg+xml")&&(l.width=a,l.height=a);let c=l.naturalWidth/l.naturalHeight,t=Math.floor(a*.15),p=Math.ceil(t*2.5),s=a,g=a;c>1?g=Math.floor(a/c):s=Math.floor(a*c),e.width=s+2*p,e.height=g+2*p;let f=e.getContext("2d",{willReadFrequently:!0});if(!f)throw new Error("Failed to get canvas 2d context");f.fillStyle="white",f.fillRect(0,0,e.width,e.height),f.drawImage(l,p,p,s,g);let{width:m,height:I}=e,v=f.getImageData(0,0,m,I).data,i=m*I,n=new Uint8ClampedArray(i);for(let O=0;O<i;O++){let z=O*4,E=v[z]??0,X=v[z+1]??0,L=v[z+2]??0;n[O]=.299*E+.587*X+.114*L|0}let b=t,w=Math.max(1,Math.round(.12*t)),V=5,F=Se(n,m,I,b,3),C=Se(n,m,I,w,3),x=Se(n,m,I,V,1),R=f.createImageData(m,I),W=R.data;for(let O=0;O<i;O++){let z=O*4;W[z]=x[O]??0,W[z+1]=F[O]??0,W[z+2]=C[O]??0,W[z+3]=255}f.putImageData(R,0,0),e.toBlob(O=>{if(!O){u(new Error("Failed to create PNG blob"));return}r({blob:O})},"image/png")}),l.addEventListener("error",()=>{u(new Error("Failed to load image"))}),l.src=typeof o=="string"?o:URL.createObjectURL(o)})}function Me(o,e,a,r){if(r<=0)return o.slice();let u=new Uint8ClampedArray(e*a),l=new Uint32Array(e*a);for(let c=0;c<a;c++){let t=0;for(let p=0;p<e;p++){let s=c*e+p,g=o[s]??0;t+=g,l[s]=t+(c>0?l[s-e]??0:0)}}for(let c=0;c<a;c++){let t=Math.max(0,c-r),p=Math.min(a-1,c+r);for(let s=0;s<e;s++){let g=Math.max(0,s-r),f=Math.min(e-1,s+r),m=p*e+f,I=p*e+(g-1),P=(t-1)*e+f,v=(t-1)*e+(g-1),i=l[m]??0,n=g>0?l[I]??0:0,b=t>0?l[P]??0:0,w=g>0&&t>0?l[v]??0:0,V=i-n-b+w,F=(f-g+1)*(p-t+1);u[c*e+s]=Math.round(V/F)}}return u}function Se(o,e,a,r,u){if(r<=0||u<=1)return Me(o,e,a,r);let l=o,c=o;for(let t=0;t<u;t++)c=Me(l,e,a,r),l=c;return c}var So=`#version 300 es
precision mediump float;
...
`,J={measurePerformance:!1,workingSize:512,iterations:40};function Bo(o){let e=document.createElement("canvas"),a=e.getContext("2d"),r=typeof o=="string"&&o.startsWith("blob:");return new Promise((u,l)=>{if(!o||!a){l(new Error("Invalid file or canvas context"));return}let c=r&&fetch(o).then(s=>s.headers.get("Content-Type")),t=new Image;t.crossOrigin="anonymous";let p=performance.now();t.onload=async()=>{let s,g=await c;g?s=g==="image/svg+xml":typeof o=="string"?s=o.endsWith(".svg")||o.startsWith("data:image/svg+xml"):s=o.type==="image/svg+xml";let f=t.width||t.naturalWidth,m=t.height||t.naturalHeight;if(s){let _=f/m;f>m?(f=4096,m=4096/_):(m=4096,f=4096*_),t.width=f,t.height=m}let I=Math.min(f,m),v=J.workingSize/I,i=Math.round(f*v),n=Math.round(m*v);J.measurePerformance&&(console.log("[Processing Mode]"),console.log(`  Original: ${f}\xD7${m}`),console.log(`  Working: ${i}\xD7${n} (${(v*100).toFixed(1)}% scale)`),v<1&&console.log(`  Speedup: ~${Math.round(1/(v*v))}\xD7`)),e.width=f,e.height=m;let b=document.createElement("canvas");b.width=i,b.height=n;let w=b.getContext("2d");w.drawImage(t,0,0,i,n);let V=performance.now(),C=w.getImageData(0,0,i,n).data,x=new Uint8Array(i*n),R=new Uint8Array(i*n),W=0;for(let h=0,_=0;h<C.length;h+=4,_++){let D=C[h+3]===0?0:1;x[_]=D,W+=D}let O=[],z=[];for(let h=0;h<n;h++)for(let _=0;_<i;_++){let S=h*i+_;if(!x[S])continue;let D=!1;_===0||_===i-1||h===0||h===n-1?D=!0:D=!x[S-1]||!x[S+1]||!x[S-i]||!x[S+i]||!x[S-i-1]||!x[S-i+1]||!x[S+i-1]||!x[S+i+1],D?(R[S]=1,O.push(S)):z.push(S)}J.measurePerformance&&(console.log(`[Mask Building] Time: ${(performance.now()-V).toFixed(2)}ms`),console.log(`  Shape pixels: ${W} / ${i*n} (${(W/(i*n)*100).toFixed(1)}%)`),console.log(`  Interior pixels: ${z.length}`),console.log(`  Boundary pixels: ${O.length}`));let E=Uo(x,R,new Uint32Array(z),new Uint32Array(O),i,n),X=performance.now(),L=Vo(E,x,R,i,n);J.measurePerformance&&console.log(`[Poisson Solve] Time: ${(performance.now()-X).toFixed(2)}ms`);let K=0,H;for(let h=0;h<z.length;h++){let _=z[h];L[_]>K&&(K=L[_])}let $=document.createElement("canvas");$.width=i,$.height=n;let ie=$.getContext("2d"),Q=ie.createImageData(i,n);for(let h=0;h<n;h++)for(let _=0;_<i;_++){let S=h*i+_,D=S*4;if(!x[S])Q.data[D]=255,Q.data[D+1]=255,Q.data[D+2]=255,Q.data[D+3]=0;else{let oe=255*(1-L[S]/K);Q.data[D]=oe,Q.data[D+1]=oe,Q.data[D+2]=oe,Q.data[D+3]=255}}ie.putImageData(Q,0,0),a.imageSmoothingEnabled=!0,a.imageSmoothingQuality="high",a.drawImage($,0,0,i,n,0,0,f,m);let G=a.getImageData(0,0,f,m),ee=document.createElement("canvas");ee.width=f,ee.height=m;let j=ee.getContext("2d");j.drawImage(t,0,0,f,m);let re=j.getImageData(0,0,f,m);for(let h=0;h<G.data.length;h+=4){let _=re.data[h+3],S=G.data[h+3];_===0?(G.data[h]=255,G.data[h+1]=0):(G.data[h]=S===0?0:G.data[h],G.data[h+1]=_),G.data[h+2]=255,G.data[h+3]=255}a.putImageData(G,0,0),H=G,e.toBlob(h=>{if(!h){l(new Error("Failed to create PNG blob"));return}if(J.measurePerformance){let _=performance.now()-p;if(console.log(`[Total Processing Time] ${_.toFixed(2)}ms`),v<1){let S=_*Math.pow(f*m/(i*n),1.5);console.log(`[Estimated time at full resolution] ~${S.toFixed(0)}ms`),console.log(`[Time saved] ~${(S-_).toFixed(0)}ms (${Math.round(S/_)}\xD7 faster)`)}}u({imageData:H,pngBlob:h})},"image/png")},t.onerror=()=>l(new Error("Failed to load image")),t.src=typeof o=="string"?o:URL.createObjectURL(o)})}function Uo(o,e,a,r,u,l){let c=a.length,t=new Int32Array(c*4);for(let p=0;p<c;p++){let s=a[p],g=s%u,f=Math.floor(s/u);t[p*4+0]=g<u-1&&o[s+1]?s+1:-1,t[p*4+1]=g>0&&o[s-1]?s-1:-1,t[p*4+2]=f>0&&o[s-u]?s-u:-1,t[p*4+3]=f<l-1&&o[s+u]?s+u:-1}return{interiorPixels:a,boundaryPixels:r,pixelCount:c,neighborIndices:t}}function Vo(o,e,a,r,u){let l=J.iterations,c=.01,t=new Float32Array(r*u),{interiorPixels:p,neighborIndices:s,pixelCount:g}=o,f=performance.now(),m=1.9,I=[],P=[];for(let v=0;v<g;v++){let i=p[v],n=i%r,b=Math.floor(i/r);(n+b)%2===0?I.push(v):P.push(v)}for(let v=0;v<l;v++){for(let i of I){let n=p[i],b=s[i*4+0],w=s[i*4+1],V=s[i*4+2],F=s[i*4+3],C=0;b>=0&&(C+=t[b]),w>=0&&(C+=t[w]),V>=0&&(C+=t[V]),F>=0&&(C+=t[F]);let x=(c+C)/4;t[n]=m*x+(1-m)*t[n]}for(let i of P){let n=p[i],b=s[i*4+0],w=s[i*4+1],V=s[i*4+2],F=s[i*4+3],C=0;b>=0&&(C+=t[b]),w>=0&&(C+=t[w]),V>=0&&(C+=t[V]),F>=0&&(C+=t[F]);let x=(c+C)/4;t[n]=m*x+(1-m)*t[n]}}if(J.measurePerformance){let v=performance.now()-f;console.log(`[Optimized Poisson Solver (SOR \u03C9=${m})]`),console.log(`  Working size: ${r}\xD7${u}`),console.log(`  Iterations: ${l}`),console.log(`  Time: ${v.toFixed(2)}ms`),console.log(`  Interior pixels processed: ${g}`),console.log(`  Speed: ${(l*g/(v*1e3)).toFixed(2)} Mpixels/sec`)}return t}var ko={none:0,circle:1,daisy:2,diamond:3,metaballs:4};var Io=`#version 300 es
precision mediump float;
...
`,Fo={classic:0,gooey:1,holes:2,soft:3},Ro={square:0,hex:1};var Mo=`#version 300 es
precision mediump float;
...
`,Oo={dots:0,ink:1,sharp:2};var Ue={maxColorCount:6},Do=`#version 300 es
precision mediump float;
...
// Smoke UV setup
  ...
  float smokeMask = 0.;
  for (int i = 1; i < ${Ue.maxColorCount+1}; i++) {
    if (i > int(u_colorsCount)) break;
